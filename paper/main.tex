...
ML workloads are computation graphs where performance depends on dataflow/tiling, KV cache~\cite{vllm2023}, and compute--memory--network balance.
Self-attention scales as $O(n^2)$ in sequence length, making memory bandwidth the dominant bottleneck for long-context LLMs: a 128k-token KV cache on a 70B-parameter model can exceed 40\,GB, saturating HBM capacity and forcing paged or offloaded storage~\cite{vllm2023}.
MoE architectures and dynamic inference (e.g., speculative decoding, early exit) introduce input-dependent control flow that violates the static-graph assumption underlying most analytical models.
LLM inference splits into compute-bound prefill and memory-bound decode~\cite{splitwise2024,sarathi2024,orca2022}; the ratio between these phases varies by request length, so accurate prediction requires modeling both regimes and their scheduling interaction.
...
\textbf{Methodology--platform pairings.}\label{subsec:by-methodology} Platform constrains methodology: accelerators use analytical models~\cite{timeloop2019,maestro2019}; GPUs span all five types; distributed systems need trace-driven simulation~\cite{astrasim2023,vidur2024}; edge relies on ML-augmented~\cite{nnmeter2021,litepred2024}; CPUs remain least studied, with only three tools providing any CPU support (Table~\ref{tab:taxonomy-matrix}).
This methodology--platform coupling means that cross-platform comparison is inherently difficult: an analytical model validated on an NPU cannot be directly applied to GPU workloads without re-deriving memory hierarchy parameters.
Errors propagate across abstraction levels (Figure~\ref{fig:abstraction-levels}): kernel-level tools achieve 2--3\% error, but composition to full models amplifies this to 5--12\%, and system-level orchestration adds further noise to reach 5--15\%.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\textbf{Workload coverage.}\label{subsec:workload-coverage} Of 14 tools, 9 validate only on CNNs; post-2023 tools target transformers/LLMs but \textbf{none validates on diffusion or dynamic inference}~\cite{dynamicreasoning2026}; only Frontier~\cite{frontier2025} covers MoE; and none spans the full kernel-to-system stack.
This workload-coverage fragmentation means researchers must chain multiple tools---e.g., NeuSight for kernel latency, then ASTRA-sim for collective communication---introducing interface mismatches and compounding prediction error at each boundary.
...
\textbf{(3)~Hardware transfer:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) and congestion modeling~\cite{astrasim2023,triosim2025,ns3_2010} remain unsolved. \textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; portable formats~\cite{chakra2023} and continuous validation~\cite{mlperfpower2025} are needed. \textbf{(5)~Reproducibility:} Dependency rot is a practical barrier---nn-Meter failed due to incompatible libraries; containerized environments and CI validation are needed to prevent tools from becoming unusable.
