...
We classify each paper along two primary dimensions: \emph{methodology type} (analytical, cycle-accurate simulation, trace-driven simulation, ML-augmented, or hybrid) and \emph{target platform} (DNN accelerator, GPU, distributed system, edge device, or CPU).
Secondary dimensions include workload coverage, prediction targets, and reported accuracy metrics.
...
We organize the literature along two primary dimensions: \emph{methodology type} and \emph{target platform}.
Figure~\ref{fig:taxonomy-overview} illustrates how these dimensions intersect.
This methodology-centric taxonomy helps practitioners select appropriate tools for their use case and helps researchers identify gaps in the current landscape.
...
\subsection{By Methodology Type}
...
For DNN accelerators, Timeloop~\cite{timeloop2019} models data movement across memory hierarchies for any valid loop-nest mapping, achieving 5--10\% accuracy versus RTL at 2000$\times$ speedup.
...
AMALI's 23.6\% MAPE illustrates the challenge: reducing GPU LLM inference modeling error from 127\% to 24\% through improved memory hierarchy treatment, but significant residual error remains from unmodeled dynamic effects---this represents the fundamental difficulty of analytical approaches for complex workloads, not a quality issue~\cite{amali2025}.
...
ASTRA-sim~\cite{astrasim2023} models distributed training end-to-end using Chakra execution traces~\cite{chakra2023}, with pluggable compute, memory, and network backends, achieving 5--15\% accuracy versus real clusters.
...
Note that some tools in this category use ML internally (e.g., VIDUR uses random forests for kernel latency prediction), blurring the boundary with hybrid approaches.
...
However, they may fail silently outside their training distribution (e.g., CNN-trained models applied to transformers) and provide limited interpretability for design insight.
nn-Meter's paper-reported $<$1\% MAPE cannot be independently verified, as the tool's pre-trained predictors fail with current scikit-learn versions due to pickle serialization changes---a cautionary example of how ML-augmented approaches can become irreproducible.
...
Timeloop~\cite{timeloop2019} analytically computes data reuse, latency, and energy from loop-nest representations, achieving 5--10\% accuracy versus RTL simulation at 2000$\times$ speedup.
...
Sparseloop~\cite{sparseloop2022} extends to sparse tensor operations, critical for efficient transformer inference where attention matrices exhibit structured sparsity.

\textbf{Simulation approaches.}
PyTorchSim~\cite{pytorchsim2025} integrates PyTorch 2 with NPU simulation supporting custom RISC-V ISA and systolic arrays, bridging the gap between ML frameworks and hardware simulation.
...
uPIMulator~\cite{upimulator2024} provides cycle-accurate PIM simulation for UPMEM devices.
AttAcc~\cite{attacc2024} and NeuPIMs~\cite{neupims2024} target PIM-based attention acceleration for transformers, while PAISE~\cite{paise2025} addresses PIM-accelerated LLM inference scheduling.
These tools remain in early stages compared to the mature DNN accelerator modeling ecosystem.
...
Habitat~\cite{habitat2021} decomposes execution into analytically-modeled compute and memory components using wave scaling analysis, achieving 11.8\% error across GPU generations.
Note that direct comparison between NeuSight and Habitat requires caution: NeuSight evaluates on 2023--2025 hardware (H100) with LLM workloads, while Habitat was designed for earlier GPUs with CNN/RNN workloads---the reported ``50$\times$ improvement'' reflects different evaluation conditions rather than purely methodological advances.
...
It achieves 5--15\% accuracy versus real clusters and enables exploration of parallelization strategies at scale.
SimAI~\cite{simai2025} provides full-stack LLM training simulation at Alibaba Cloud scale, achieving 98.1\% alignment with production results.
...
Echo~\cite{echo2024} simulates distributed training at scale.
TrioSim~\cite{triosim2025} provides lightweight multi-GPU simulation.
PRISM~\cite{prism2025} uses probabilistic models for training performance prediction at 10K+ GPU scale.
...
MAD Max~\cite{madmax2024} extends analytical modeling to distributed systems.
...
Sailor~\cite{sailor2025} addresses automated parallelism selection over heterogeneous clusters.
...
ESM~\cite{esm2025} provides a framework for building effective surrogate models for hardware-aware neural architecture search.
...
The kernel-to-end-to-end composition problem remains unsolved.
