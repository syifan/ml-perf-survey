...
The gap between kernel-level accuracy (2--3\% for NeuSight) and system-level accuracy (5--15\% for ASTRA-sim) reflects both the inherent difficulty of system modeling and the error accumulated through composition.
...
However, it does not report accuracy against fabricated hardware, and its cycle-accurate approach inherits the speed limitations of simulation-based methods, making it impractical for large-model evaluation.
...
SynPerf~\cite{synperf2025} takes a complementary approach, synthesizing high-performance GPU kernels via pipeline decomposition---using performance models to guide kernel generation rather than merely evaluating existing kernels, demonstrating that accurate performance prediction enables not just evaluation but also automated optimization.
...
SimAI~\cite{simai2025} provides full-stack LLM training simulation at Alibaba Cloud scale, modeling compute, memory, network, and collective communication in an integrated framework that achieves 98.1\% alignment with production results.
...
Echo~\cite{echo2024} simulates distributed training at scale by combining analytical compute models with packet-level network simulation, enabling evaluation of collective communication algorithms and network topology effects without requiring cluster access; however, it focuses on communication modeling rather than end-to-end training fidelity, and does not report absolute accuracy against real hardware measurements.
TrioSim~\cite{triosim2025} provides lightweight multi-GPU DNN training simulation through selective fidelity---modeling compute, memory, and inter-GPU communication at different abstraction levels to balance speed and accuracy.
While TrioSim enables rapid what-if analysis for multi-GPU configurations, it does not report accuracy baselines against real hardware, making it suitable for relative comparison between configurations but not for absolute performance prediction.
PRISM~\cite{prism2025} uses probabilistic models for training performance prediction at 10K+ GPU scale, addressing the challenge that deterministic simulators cannot capture the stochastic performance variation (network congestion, stragglers, thermal throttling) observed in large-scale training; its probabilistic approach produces prediction intervals rather than point estimates, which better reflects the inherent uncertainty in large-scale distributed systems.
...
MAD Max~\cite{madmax2024} extends analytical modeling to distributed systems by decomposing training time into compute, communication, and memory components per parallelism dimension, enabling rapid evaluation of parallelism configurations without simulation; its analytical approach achieves fast evaluation but shares the accuracy limitations of single-device analytical models, compounded by approximations of network contention and load imbalance.
...
This pattern suggests that \emph{verifiable moderate accuracy} matters more to practitioners than \emph{unverifiable high accuracy}.
