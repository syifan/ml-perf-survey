...
ML workloads are computation graphs where performance depends on dataflow/tiling, KV cache~\cite{vllm2023}, and compute--memory--network balance; self-attention scales as $O(n^2)$ in sequence length, making memory bandwidth the dominant bottleneck for long-context LLMs whose KV caches can exceed 40\,GB. MoE architectures and dynamic inference (e.g., speculative decoding, early exit) add input-dependent control flow that violates the static-graph assumption underlying most analytical models. LLM inference splits into compute-bound prefill and memory-bound decode~\cite{splitwise2024,sarathi2024,orca2022}; accurate prediction requires modeling both regimes and their scheduling interaction. Five modeling types span accuracy--speed trade-offs: \textbf{analytical}~\cite{williams2009roofline} ($\mu$s), \textbf{cycle-accurate}~\cite{gpgpusim2009,accelsim2020} ($10^3$--$10^4\times$ slowdown), \textbf{trace-driven}~\cite{astrasim2023,vidur2024} (min.), \textbf{ML-augmented}~\cite{nnmeter2021} (ms), and \textbf{hybrid}~\cite{neusight2025,habitat2021}.
...
\textbf{Methodology--platform pairings.}\label{subsec:by-methodology} Platform constrains methodology: accelerators use analytical models~\cite{timeloop2019,maestro2019}; GPUs span all five types; distributed systems need trace-driven simulation~\cite{astrasim2023,vidur2024}; edge relies on ML-augmented~\cite{nnmeter2021,litepred2024}; CPUs remain least studied, with only three tools providing any CPU support (Table~\ref{tab:taxonomy-matrix}). This coupling makes cross-platform comparison difficult, as models validated on one platform cannot transfer without re-deriving memory hierarchy parameters. Errors propagate across abstraction levels (Figure~\ref{fig:abstraction-levels}): kernel 2--3\%, model 5--12\%, system 5--15\%.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\textbf{Workload coverage.}\label{subsec:workload-coverage} Of 14 tools, 9 validate only on CNNs; post-2023 tools target transformers/LLMs but \textbf{none validates on diffusion or dynamic inference}~\cite{dynamicreasoning2026}; only Frontier~\cite{frontier2025} covers MoE; none spans the full kernel-to-system stack, forcing researchers to chain tools (e.g., NeuSight for kernels, then ASTRA-sim for collectives) with interface mismatches that compound prediction error.
