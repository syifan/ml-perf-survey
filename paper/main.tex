...
Having surveyed the landscape of ML-based performance modeling approaches, we now provide a comparative analysis across key dimensions.
...
\begin{table*}[t]
\centering
\caption{Comparative analysis of representative ML-based performance models across key dimensions.}
\label{tab:comparison-summary}
\small
\begin{tabular}{lcccccc}
\toprule
\textbf{Model} & \textbf{Accuracy} & \textbf{Training Data} & \textbf{Adaptation Cost} & \textbf{Generalization} & \textbf{Interpretability} & \textbf{Inference Time} \\
\midrule
\multicolumn{7}{l}{\textit{Classical ML}} \\
nn-Meter~\cite{nnmeter2021} & $<$1\% MAPE & 1K/kernel & Hours/device & Device-specific & Medium & $<$1ms \\
XGBoost (TVM) & 20\% MAPE & 10K+ & Online & Operator-level & Medium & $<$1ms \\
\midrule
\multicolumn{7}{l}{\textit{Deep Learning}} \\
NeuSight~\cite{neusight2025} & 2.3\% MAPE & 100K+ & Pre-trained & Cross-GPU & Low & $<$10ms \\
Habitat~\cite{habitat2021} & 11.8\% MAPE & Runtime & None (requires GPU) & Cross-GPU & Medium & Runtime \\
\midrule
...
TVM's AutoTVM achieves approximately 20\% MAPE, sufficient for guiding autotuning search.
...
CALO-GNN provides calibrated uncertainty estimates, but such techniques are not yet standard.
