...
We searched ACM DL, IEEE Xplore, Semantic Scholar, and arXiv (MICRO, ISCA, HPCA, ASPLOS, MLSys, OSDI, SOSP). From 287 candidates, 53 papers (2016--2026) plus 12 foundational works were classified by methodology, platform, and abstraction level~\cite{rakhshanfar2021survey}, excluding proprietary tools, infrastructure~\cite{binkert2011gem5,sst2012}, compiler cost models~\cite{halide2013,mlir2020,triton2019}, and schedulers~\cite{pollux2021,sia2023}.
...
No single tool spans kernel execution through serving SLAs. Figure~\ref{fig:pipeline-architecture} shows five layers---hardware design, kernel prediction, \textbf{model composition (CRITICAL GAP)}, distributed training, and serving---where 5--9\% kernel MAPE grows to 10--28\% at model level via launch overhead. Bridging these layers requires solving three integration challenges: (i)~\emph{interface heterogeneity}---each tool defines its own input format (YAML, Chakra traces~\cite{chakra2023}, request logs), so layer boundaries demand ad-hoc conversion; (ii)~\emph{calibration mismatch}---analytical models assume steady-state execution while transient effects (JIT warm-up, memory fragmentation) dominate short kernels; and (iii)~\emph{feedback loops}---serving systems adapt scheduling based on predicted latency, creating circular dependencies that static pipelines cannot capture.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
