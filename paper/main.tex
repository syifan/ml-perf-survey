...
ML workloads are computation graphs where performance depends on dataflow/tiling, KV cache~\cite{vllm2023}, and compute--memory--network balance; LLM inference splits into compute-bound prefill and memory-bound decode~\cite{splitwise2024,sarathi2024,orca2022}.
This phase asymmetry forces serving systems to disaggregate or chunk requests~\cite{distserve2024}, creating modeling challenges that span both compute and memory subsystems.
On the hardware side, multi-chip module (MCM) packaging enables disaggregated compute where accelerator dies, memory stacks, and interconnect chiplets are composed into heterogeneous packages, invalidating monolithic roofline assumptions~\cite{williams2009roofline}.
High-bandwidth memory (HBM3/HBM3E) alleviates but does not eliminate the memory bandwidth wall; at 4.8\,TB/s per stack the bottleneck shifts to on-package interconnect and thermal limits, requiring models that capture die-to-die transfer costs.
Processing-in-memory and near-memory compute architectures~\cite{upimulator2024,attacc2024,neupims2024} further complicate prediction by coupling compute throughput to memory access patterns, demanding new per-bank modeling abstractions absent from current tools.
...
\textbf{Methodology--platform pairings.}\label{subsec:by-methodology} Platform constrains methodology: accelerators use analytical models~\cite{timeloop2019,maestro2019}; GPUs span all five types; distributed systems need trace-driven simulation~\cite{astrasim2023,vidur2024}; edge relies on ML-augmented~\cite{nnmeter2021,litepred2024}. Errors propagate (Figure~\ref{fig:abstraction-levels}): kernel 2--3\%, model 5--12\%, system 5--15\%.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\end{tabular}\end{table*}\textbf{DNN accelerators and GPUs.} Analytical tools---Timeloop~\cite{timeloop2019}, MAESTRO~\cite{maestro2019}, Sparseloop~\cite{sparseloop2022}, PIM tools~\cite{upimulator2024,attacc2024,neupims2024,paise2025}, ArchGym~\cite{archgym2023}---enumerate mappings; cycle-accurate simulators~\cite{gpgpusim2009,accelsim2020} achieve 0.90--0.97 IPC correlation at $10^3$--$10^4\times$ slowdown; hybrid tools~\cite{neusight2025,habitat2021,amali2025,tvm2018,ansor2020,tlp2023,life2025,hermes2025,omniwise2025,swizzleperf2025,synperf2025,tenset2021} trade accuracy for speed. \textbf{Distributed/serving:} ASTRA-sim~\cite{astrasim2023}, SimAI~\cite{simai2025}, VIDUR~\cite{vidur2024}, Lumos~\cite{lumos2025}, and others~\cite{echo2024,paleo2017,distserve2024,frontier2025,podattention2025,aqua2025,throttllem2025,sailor2025} cover training and serving; LitePred~\cite{litepred2024} and HELP~\cite{help2021} cover mobile~\cite{esm2025,latencypredictorsnas2024}. A cross-cutting limitation is \emph{scope rigidity}: analytical tools miss dynamic sparsity, cycle-accurate simulators are too costly for sweeps, and trace-driven tools assume deterministic replay.
...
No single tool spans kernel execution through serving SLAs. Figure~\ref{fig:pipeline-architecture} shows five layers---hardware design, kernel prediction, \textbf{model composition (CRITICAL GAP)}, distributed training, and serving---where 5--9\% kernel MAPE grows to 10--28\% at model level. Three integration challenges remain: (i)~\emph{interface heterogeneity}---each tool defines its own format (YAML, Chakra traces~\cite{chakra2023}, request logs); (ii)~\emph{calibration mismatch}---analytical models assume steady-state while transient effects dominate short kernels; and (iii)~\emph{feedback loops}---serving systems adapt scheduling based on predicted latency, creating circular dependencies.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\end{tikzpicture}}\caption{Unified pipeline across five layers. Layer~3 (dashed) is the critical gap where 5--15\% unmodeled error accumulates.}\label{fig:pipeline-architecture}\end{figure}
...
\textbf{(3)~Hardware transfer:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) and congestion modeling~\cite{astrasim2023,triosim2025,ns3_2010} remain unsolved.
Domain adaptation techniques---fine-tuning learned latency models on small target-hardware datasets while reusing source-hardware feature extractors---offer a promising path but require architecture-aware embedding spaces that current tools lack.
\textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; portable formats~\cite{chakra2023} and continuous validation~\cite{mlperfpower2025} are needed.
A community benchmark suite spanning kernel, model, and system levels---with reference hardware measurements on at least three GPU generations---would enable apples-to-apples accuracy comparisons and accelerate tool development.
\textbf{(5)~Reproducibility:} Dependency rot is a practical barrier---nn-Meter failed due to incompatible libraries; containerized environments and CI validation are needed to prevent tools from becoming unusable.
Continuous integration pipelines that rebuild simulator toolchains nightly and run regression tests against golden traces would catch compatibility breakage before it silently invalidates published results.
\section{Conclusion}\label{sec:conclusion}
We survey 22 ML performance tools and evaluate five against a 28-scenario benchmark, finding self-reported accuracy unreliable (NeuSight: 2.3\% claimed vs.\ 5.87--27.10\%; nn-Meter: no output). The 5--15\% kernel-to-model composition gap dominates total error; validated composition models and continuous validation are the most pressing needs.
More broadly, realizing the unified five-layer pipeline (Figure~\ref{fig:pipeline-architecture}) requires a coordinated research agenda: standardized intermediate representations at each layer boundary, open benchmark suites with ground-truth hardware measurements, and community infrastructure for continuous regression testing---collectively lowering the barrier to rigorous, reproducible performance prediction across the rapidly diversifying hardware landscape.
