...
\begin{abstract}
We survey 22 performance modeling tools from 53 papers (2016--2026) and independently evaluate five---NeuSight, ASTRA-sim, VIDUR, Timeloop, nn-Meter---across 146 GPU configurations, collective benchmarks, LLM serving simulations, energy validation, and reproducibility testing. Three findings emerge: (1)~self-reported accuracy is unreliable---NeuSight claims 2.3\% MAPE but we measure 5.87--27.10\%, while nn-Meter ($<$1\% claimed) produces no output due to dependency rot; (2)~the five tools are complementary but disjoint across kernel prediction, communication simulation, LLM serving, accelerator design, and edge inference, motivating a unified pipeline; (3)~the kernel-to-model composition gap (2--9\% kernel error growing to 10--28\% model error) dominates total prediction error, yet no existing tool addresses this layer.
\end{abstract}
...
Domain-specific architectures~\cite{hennessy2019golden,tpuv1_2017,tpuv4_2023} make performance prediction critical, yet no prior work examines \emph{why} certain approaches succeed---analytical~\cite{timeloop2019,maestro2019}, trace-driven~\cite{astrasim2023,vidur2024}, hybrid~\cite{neusight2025}---or how errors propagate across the stack; existing surveys cover ML \emph{techniques} for modeling~\cite{granite2022} or specific hardware.
We contribute: (1)~a \textbf{28-scenario LLM benchmark suite} revealing 50\% of scenarios lack tool support; (2)~\textbf{independent verification} of five tools showing claimed error rates are overstated by $2$--$4\times$; (3)~a \textbf{unified pipeline} identifying the composition gap; and (4)~a \textbf{research agenda} for composition modeling, unified formats, and continuous validation.
\begin{figure}[t]
...
\end{tikzpicture}}
\caption{Evolution of performance modeling tools (2016--2026).}\label{fig:timeline}
\end{figure}
\section{Survey Methodology}\label{sec:methodology}
We searched ACM DL, IEEE Xplore, Semantic Scholar, and arXiv (MICRO, ISCA, HPCA, ASPLOS, MLSys, OSDI, SOSP). From 287 candidates, 53 papers (2016--2026) plus 12 foundational works were classified by \emph{methodology type}, \emph{target platform}, and \emph{abstraction level}~\cite{rakhshanfar2021survey}, excluding proprietary tools, simulation infrastructure~\cite{binkert2011gem5,sst2012}, compiler cost models~\cite{halide2013,mlir2020,triton2019}, and cluster schedulers~\cite{pollux2021,sia2023}.
\section{Background}\label{sec:background}
ML workloads are computation graphs amenable to analytical modeling; MoE and dynamic inference add input-dependent control flow~\cite{pytorch2019,tensorflow2016}. Performance depends on dataflow/tiling, KV cache~\cite{vllm2023}, and compute--memory--network balance; LLM inference splits into compute-bound prefill and memory-bound decode~\cite{splitwise2024,sarathi2024,orca2022}. Five types span accuracy--speed trade-offs: \textbf{analytical}~\cite{williams2009roofline} ($\mu$s), \textbf{cycle-accurate}~\cite{gpgpusim2009,accelsim2020} ($10^3$--$10^4\times$ slowdown), \textbf{trace-driven}~\cite{astrasim2023,vidur2024} (min.), \textbf{ML-augmented}~\cite{nnmeter2021} (ms), and \textbf{hybrid}~\cite{neusight2025,habitat2021}.
...
\end{tabular}
\end{table*}
Three gaps emerge (Figure~\ref{fig:tool-architecture}): trace-driven replay is exclusive to distributed systems, edge devices lack hybrid methods, and no ML-augmented tool targets distributed settings.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\draw[arr] (analytical) -- (hybrid); \draw[arr] (mlmodel) -- (hybrid); \draw[arr, dashed, gray] (simulator) -- node[lbl, right] {valid.} (hybrid); \draw[arr] (hybrid) -- (system); \draw[arr] (analytical) |- (system); \draw[arr] (system) -- (output);
...
Platform constrains methodology: accelerators use analytical models~\cite{timeloop2019,maestro2019}; GPUs span all five types; distributed systems need trace-driven simulation~\cite{astrasim2023,vidur2024}; edge relies on ML-augmented~\cite{nnmeter2021,litepred2024}; CPUs remain least studied. Errors propagate (Figure~\ref{fig:abstraction-levels}): kernel 2--3\%, model 5--12\%, system 5--15\%.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
Of 14 tools, 9 validate only on CNNs; post-2023 tools target transformers/LLMs but \textbf{none validates on diffusion or dynamic inference}~\cite{dynamicreasoning2026}; only Frontier~\cite{frontier2025} covers MoE; none spans the full kernel-to-system stack.
...
\end{tabular}
\end{table*}
\textbf{DNN accelerators and GPUs.} Timeloop~\cite{timeloop2019} enumerates loop-nest mappings (5--10\%); MAESTRO~\cite{maestro2019} and Sparseloop~\cite{sparseloop2022} extend to data-centric and sparse tensors; PIM tools~\cite{upimulator2024,attacc2024,neupims2024,paise2025} and ArchGym~\cite{archgym2023} represent newer approaches. Cycle-accurate simulators (GPGPU-Sim~\cite{gpgpusim2009}, Accel-Sim~\cite{accelsim2020}) achieve 0.90--0.97 IPC correlation at $10^3$--$10^4\times$ slowdown; hybrid tools NeuSight~\cite{neusight2025} (2.3\%), Habitat~\cite{habitat2021} (11.8\%), AMALI~\cite{amali2025} (23.6\%), and TVM/Ansor/TLP~\cite{tvm2018,ansor2020,tlp2023,life2025,hermes2025,omniwise2025,swizzleperf2025,synperf2025,tenset2021} trade accuracy for speed. \textbf{Distributed and serving:} ASTRA-sim~\cite{astrasim2023} (5--15\%), SimAI~\cite{simai2025} (1.9\%), VIDUR~\cite{vidur2024}, Lumos~\cite{lumos2025} (3.3\%), and others~\cite{echo2024,paleo2017,distserve2024,frontier2025,podattention2025,aqua2025,throttllem2025,sailor2025} cover distributed and serving; LitePred~\cite{litepred2024} (0.7\%) and HELP~\cite{help2021} (1.9\%) cover mobile~\cite{esm2025,latencypredictorsnas2024}.
...
We introduce \textbf{accuracy-centered independent verification}: an \textbf{LLM benchmark suite} of 28 scenarios (Sec.~\ref{subsec:benchmark-suite}) defines standardized coverage, while \textbf{independent experiments} deploy each tool from its public artifact and measure accuracy under controlled conditions---replacing self-reported claims with reproducible third-party evaluation.
...
Training scenarios T1--T3 cover the three canonical parallelism dimensions (data, tensor, pipeline); T4 targets computation-graph modifications (FP8, LoRA, MoE).
Inference scenarios span single-request latency (I1) through batched serving (I2), KV cache management (I3), multi-model GPU sharing (I4), and production optimizations like speculative decoding and disaggregated serving~\cite{splitwise2024} (I5).
...
Coverage was verified by consulting tool documentation, configuration schemas, and attempting actual runs where feasible; post-hoc workarounds were not counted as ``supported.''
...
Our platform lacks discrete GPUs, so we verify \emph{claim reproducibility} (whether published accuracy is achievable from a tool's own artifacts), \emph{internal consistency} (e.g., Reduce-Scatter $\approx$ 0.5$\times$ All-Reduce), and \emph{relative ranking} (e.g., Sarathi vs.\ vLLM latency) rather than absolute accuracy.
The $N=5$ sample provides case-study-level findings; each tool was evaluated within its intended scope (NeuSight on its 146 artifact configurations; ASTRA-sim on Ring All-Reduce at 8 NPUs; VIDUR on Llama-2-7B at QPS 2.0).
Future work should test at larger scale and with newer architectures.
...
Our venue-focused search may under-represent industry tools; we exclude proprietary tools and lack discrete GPUs for absolute accuracy verification. The 28-scenario suite cannot cover every deployment pattern (e.g., RAG, multi-modal, RLHF are not yet included).

\textbf{Internal validity.}
We evaluate 5 of 22 tools (one per methodology type), with NeuSight analyzed via its own prediction/label pairs rather than independent measurements. Per-device sample sizes vary (3--18 configurations); we report both mean and worst-case APE to mitigate this.

\textbf{Construct validity.}
Our evaluation prioritizes accuracy; tools may provide additional value (e.g., Timeloop's energy breakdown, ASTRA-sim's what-if analysis) not captured here. The ternary coverage criterion does not capture partial-support quality.

\textbf{Temporal validity.}
Results reflect tool state as of January 2026. Core findings about structural coverage gaps reflect design choices likely to persist; the extensible benchmark suite is provided as a living document in supplementary material.

\section{Toward a Unified Simulation Pipeline}\label{sec:unified-pipeline}
No single tool spans kernel execution through serving SLAs. Figure~\ref{fig:pipeline-architecture} shows five layers: hardware design (Timeloop), kernel prediction (NeuSight), \textbf{model composition (CRITICAL GAP)}, distributed training (ASTRA-sim), and serving (VIDUR). NeuSight's 5--9\% kernel MAPE grows to 10--28\% at model level via launch overhead; common formats and cross-hardware transfer remain unsolved.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\addplot[fill=blue!50, draw=blue!70] coordinates {(2016,2) (2018,2) (2020,4) (2022,3) (2024,2) (2026,0)}; \addplot[fill=orange!50, draw=orange!70] coordinates {(2016,0) (2018,1) (2020,0) (2022,2) (2024,3) (2026,0)};
\addplot[fill=red!50, draw=red!70] coordinates {(2016,0) (2018,0) (2020,1) (2022,1) (2024,8) (2026,2)}; \addplot[fill=green!50, draw=green!70] coordinates {(2016,0) (2018,0) (2020,1) (2022,0) (2024,2) (2026,1)};
...
\end{figure}
\textbf{(3)~Hardware transfer and network fidelity:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) remains unsolved; analytical abstractions omit congestion~\cite{astrasim2023,triosim2025,ns3_2010}. \textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; common benchmarks, portable formats (ONNX, Chakra~\cite{chakra2023}), and continuous validation~\cite{mlperfpower2025} are needed.
\section{Conclusion}\label{sec:conclusion}
We survey 22 ML performance tools and independently evaluate five against a 28-scenario LLM benchmark suite, finding self-reported accuracy unreliable (NeuSight: 2.3\% claimed vs.\ 5.87--27.10\% measured; nn-Meter: no output). The tools are complementary but disjoint, and the 5--15\% kernel-to-model composition gap dominates total error; validated composition models, benchmark-driven development, and continuous validation are the most pressing needs.
