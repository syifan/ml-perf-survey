...
Domain-specific architectures~\cite{hennessy2019golden,tpuv1_2017,tpuv4_2023} make performance prediction critical for ML deployment, yet no prior work examines \emph{why} certain approaches succeed---analytical~\cite{timeloop2019,maestro2019}, trace-driven~\cite{astrasim2023,vidur2024}, hybrid~\cite{neusight2025}---or how errors propagate across the stack; existing surveys cover ML \emph{techniques} for modeling~\cite{granite2022} or specific hardware~\cite{timeloop2019}.
We contribute an \textbf{accuracy-centered verification framework} with an \textbf{LLM-focused benchmark suite}, replacing self-reported accuracy with reproducible evaluation showing claimed error rates are overstated by $2$--$4\times$:
...
Platform constrains methodology: accelerators use analytical models~\cite{timeloop2019,maestro2019}; GPUs span all five types; distributed systems need trace-driven simulation~\cite{astrasim2023,vidur2024}; edge devices rely on ML-augmented~\cite{nnmeter2021,litepred2024}; CPUs~\cite{concorde2025,granite2022} remain least studied. Errors propagate (Figure~\ref{fig:abstraction-levels}): kernel 2--3\%, model 5--12\%, system 5--15\%.
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{%
...
\textbf{Distributed, serving, and edge.} ASTRA-sim~\cite{astrasim2023} replays Chakra traces~\cite{chakra2023} (5--15\%); SimAI~\cite{simai2025} models NCCL reductions (1.9\%); VIDUR~\cite{vidur2024} simulates request-level LLM serving; Lumos~\cite{lumos2025} (3.3\%), Echo~\cite{echo2024}, Paleo~\cite{paleo2017}, and other tools~\cite{distserve2024,frontier2025,podattention2025,aqua2025,throttllem2025,sailor2025} provide complementary coverage. For edge, nn-Meter~\cite{nnmeter2021} claims $<$1\% but is unverifiable (Section~\ref{sec:eval-results}); LitePred~\cite{litepred2024} achieves 0.7\% across 85 platforms; HELP~\cite{help2021} reaches 1.9\% with 10-sample meta-learning~\cite{esm2025,latencypredictorsnas2024}.
...
\textbf{(1)~Composition gap:} Kernel errors of 2--3\% yield 5--12\% model-level error (Figure~\ref{fig:error-composition}; $\sigma_{\text{model}} \approx \sigma_{\text{kernel}} \cdot \sqrt{N}$) with no validated composition pipeline.
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{%
...
\end{figure}
\textbf{(3)~Hardware transfer and network fidelity:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) remains unsolved, and tools~\cite{astrasim2023,triosim2025} use analytical network abstractions omitting congestion modeled only by NS-3~\cite{ns3_2010}.
\textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for prediction tools; the community needs common benchmarks, portable formats (ONNX, Chakra~\cite{chakra2023}), and continuous validation~\cite{mlperfpower2025}---nn-Meter's dependency rot exemplifies invalidation by software evolution~\cite{flashattention2022}.

\section{Conclusion}
\label{sec:conclusion}
We survey 22 ML performance modeling tools and independently evaluate five against a 28-scenario LLM benchmark suite, finding self-reported accuracy unreliable (NeuSight: 2.3\% claimed vs.\ 5.87--27.10\% measured; nn-Meter: no output). The five tools are complementary but disjoint, motivating a unified pipeline---yet the 5--15\% kernel-to-model composition gap exceeds kernel-level error and 50\% of modern LLM techniques lack tool support; the most pressing needs are validated composition models, benchmark-driven development, and continuous validation.
