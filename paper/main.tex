...
\end{tabular}\end{table*}\textbf{DNN accelerators and GPUs.} Analytical tools---Timeloop~\cite{timeloop2019} (5--10\%), MAESTRO~\cite{maestro2019}, Sparseloop~\cite{sparseloop2022}, PIM tools~\cite{upimulator2024,attacc2024,neupims2024,paise2025}, ArchGym~\cite{archgym2023}---enumerate mappings; cycle-accurate simulators~\cite{gpgpusim2009,accelsim2020} achieve 0.90--0.97 IPC correlation at $10^3$--$10^4\times$ slowdown; hybrid tools~\cite{neusight2025,habitat2021,amali2025,tvm2018,ansor2020,tlp2023,life2025,hermes2025,omniwise2025,swizzleperf2025,synperf2025,tenset2021} trade accuracy for speed. \textbf{Distributed/serving:} ASTRA-sim~\cite{astrasim2023}, SimAI~\cite{simai2025}, VIDUR~\cite{vidur2024}, Lumos~\cite{lumos2025}, and others~\cite{echo2024,paleo2017,distserve2024,frontier2025,podattention2025,aqua2025,throttllem2025,sailor2025} cover training and serving; LitePred~\cite{litepred2024} and HELP~\cite{help2021} cover mobile~\cite{esm2025,latencypredictorsnas2024}.

A key limitation across groups is \emph{scope rigidity}: analytical accelerator tools model fixed dataflows but cannot capture dynamic sparsity or input-dependent control flow; cycle-accurate GPU simulators provide high fidelity but prohibitive cost prevents design-space sweeps beyond a few configurations; and trace-driven distributed simulators assume deterministic replay, making them brittle under stochastic scheduling policies common in production serving systems.
...
No single tool spans kernel execution through serving SLAs. Figure~\ref{fig:pipeline-architecture} shows five layers---hardware design, kernel prediction, \textbf{model composition (CRITICAL GAP)}, distributed training, and serving---where 5--9\% kernel MAPE grows to 10--28\% at model level via launch overhead.

Bridging these layers requires solving three integration challenges: (i)~\emph{interface heterogeneity}---Timeloop consumes YAML workloads, ASTRA-sim ingests Chakra traces~\cite{chakra2023}, and VIDUR replays tokenized request logs, so layer boundaries demand ad-hoc format conversion; (ii)~\emph{calibration mismatch}---analytical models assume steady-state execution while real workloads exhibit transient effects (JIT warm-up, memory fragmentation) that dominate short kernels; and (iii)~\emph{feedback loops}---serving systems adapt scheduling (batch depth, preemption) based on predicted latency, creating circular dependencies that require co-simulation across layers~3--5.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\textbf{(3)~Hardware transfer:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) and congestion modeling~\cite{astrasim2023,triosim2025,ns3_2010} remain unsolved. \textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; portable formats~\cite{chakra2023} and continuous validation~\cite{mlperfpower2025} are needed.

\textbf{(5)~Reproducibility:} Dependency rot is a practical barrier---nn-Meter failed due to incompatible libraries, and several tools required patching; containerized environments, pinned dependencies, and CI validation against reference outputs are needed to prevent accurate tools from becoming unusable within 2--3 years.
