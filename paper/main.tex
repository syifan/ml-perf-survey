...
\emph{Structural decomposition} mirroring hardware execution outperforms black-box approaches (Timeloop's loop nests, NeuSight's tiles, VIDUR's prefill/decode), and tools with \emph{verifiable} accuracy (e.g., Timeloop's reference outputs, VIDUR's Docker-based reproduction) appear more widely adopted than tools reporting high but unverifiable accuracy (e.g., nn-Meter).
A persistent \textbf{accuracy--generality--speed trade-off} characterizes the field: simulators maximize accuracy but sacrifice speed; analytical models maximize speed but sacrifice accuracy; ML approaches achieve both but sacrifice generality.
