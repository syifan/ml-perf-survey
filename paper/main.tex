...
We introduce a \textbf{third-party evaluation focusing on accuracy and feature coverage} that addresses this gap through two components.
First, the \textbf{MLPerf-Survey-2026} benchmark suite of 36 scenarios defines standardized coverage criteria representing concrete user needs for modern LLM training, inference, and diffusion model deployment.
...
This framework is the first to systematically evaluate ML performance modeling tools through third-party testing rather than reprinting authors' own results.
...
We define the \emph{MLPerf-Survey-2026} benchmark suite comprising 36 scenarios across 9 categories representing the workloads that LLM and generative-AI practitioners need performance predictions for (Table~\ref{tab:benchmark-suite}).
The suite covers the full LLM lifecycle: pre-training with data/tensor/pipeline parallelism (T1--T3), advanced training techniques including MoE (T4), single-request inference (I1), batched serving (I2), KV cache management (I3), production optimizations (I5), and diffusion model inference (D1).
...
Each scenario specifies a concrete model (Llama-2-7B/13B/70B, GPT-2, GPT-3, Mixtral, QWen-2.5-7B/72B, DeepSeek-V2, DeepSeek-V3, Stable Diffusion XL, FLUX.1), hardware configuration (A100/H100, 1--128 GPUs), parallelism strategy, and the metric practitioners optimize (TTFT, TPOT, throughput, MFU, communication overhead, images/sec).
Training scenarios span from single-node data parallelism (T1.1: GPT-2 on 8$\times$A100) to large-scale MoE training (T4.6: DeepSeek-V3 671B on 128$\times$H100).
Inference scenarios range from single-request latency (I1.1) to MoE inference (I1.5: DeepSeek-V2 on 8$\times$H100) and production optimizations like speculative decoding (I5.1).
Diffusion scenarios (D1) cover text-to-image and image-to-image generation with SDXL and FLUX.1 models.

\textbf{Scenario selection rationale.}
The 36 scenarios were selected to reflect real deployment decisions.
...
T4 scenarios target techniques that modify the computation graph itself---FP8 changes arithmetic intensity, LoRA adds low-rank adapter layers, and MoE introduces expert routing with All-to-All communication; the addition of DeepSeek-V2 (236B, 21B active) and DeepSeek-V3 (671B, 37B active) MoE training scenarios captures the distinct expert-parallel communication patterns absent from dense-model training.
...
I1 and I2 now include QWen-2.5 (7B and 72B) and DeepSeek MoE models, reflecting the shift toward non-Llama architectures in production deployments.
...
D1 (diffusion model inference) addresses the growing deployment of image-generation models: SDXL and FLUX.1 introduce iterative denoising loops, U-Net/transformer-based architectures, and latency--quality trade-offs fundamentally different from autoregressive LLM inference.
...
Notable exclusions from full experimentation include SimAI (1.9\% claimed MAPE, but closed-source at evaluation time) and LitePred (edge-focused like nn-Meter but without public pre-trained models for the target devices we could test).
We additionally attempted deployment of 5 tools---MAESTRO, Paleo, Habitat, Accel-Sim, and ASTRA-sim's analytical backend---to document platform requirements and failure modes (Section~\ref{subsec:deployment-experience}).
...
We independently computed MAPE from the artifact's own prediction/label pairs for all 146 configurations, grouped by device and mode (training/inference).
...
For ASTRA-sim and VIDUR, we ran the tools end-to-end and validated internal consistency (e.g., deterministic outputs, correct relative ordering of collectives).
...
Against our 36-scenario suite, NeuSight achieves 5 supported and 3 partial scenarios (22\% coverage), concentrated in single-GPU inference (I1) and partial training parallelism (T1--T3).
...
Against our 36-scenario LLM benchmark suite, ASTRA-sim achieves the broadest training coverage (7 supported + 2 partial = 9 scenarios across T1--T4), but its coverage is concentrated in communication patterns rather than end-to-end training prediction.
...
Table~\ref{tab:benchmark-coverage} evaluates each tool against our 36-scenario LLM benchmark suite.
...
Of 36 scenarios, 20 (56\%) are not addressable by any evaluated tool.
The entirely uncovered scenarios include FP8 mixed-precision training (T4.1), LoRA fine-tuning (T4.2), DeepSeek MoE training (T4.5--T4.6), speculative decoding (I5.1), prefix caching (I5.2), INT4 quantized inference (I5.3), disaggregated serving (I5.4), multi-model co-location (I4.1--I4.2), and all diffusion model scenarios (D1.1--D1.3).
These represent the fastest-growing deployment patterns in production generative-AI systems.
...
This disjointness means that for 33 of 36 scenarios (92\%), practitioners have at most one tool option---and for 20 scenarios, they have none.
...
The 20 entirely uncovered scenarios cluster into four groups.
...
\emph{Multi-model gaps} (I4.1--I4.2): co-locating multiple models on shared GPUs creates memory and compute contention that requires fine-grained resource modeling beyond what any evaluated tool provides.
\emph{Diffusion model gaps} (D1.1--D1.3): iterative denoising with U-Net or transformer backbones, classifier-free guidance doubling compute, and latent-space operations create workload patterns fundamentally different from autoregressive LLM inference that no evaluated tool addresses.

\textbf{Failure mode taxonomy for uncovered scenarios.}
The 20 uncovered scenarios fail for three distinct reasons, each requiring different tool extensions.
...
\emph{Missing system-level interactions}: disaggregated serving (I5.4), multi-model co-location (I4.1--I4.2), and diffusion model pipelines (D1) create cross-component interference (network contention between prefill and decode pools, GPU memory pressure between co-located models, iterative denoising scheduling) that requires coupling otherwise independent tool components.

\textbf{Coverage concentration.}
The 16 covered scenarios concentrate in categories T1--T3 (basic parallel training) and I1--I3 (basic inference and serving).
...
Combining supported and partial scenarios, ASTRA-sim provides the broadest LLM-relevant coverage (11/36 = 31\%), followed by VIDUR (9/36 = 25\%) and NeuSight (9/36 = 25\%).
...
The union of all five tools covers only 16 of 36 scenarios (44\%), with the remaining 20 requiring entirely new tool development.
...
Even among the 16 covered scenarios, support quality is uneven.
...
These seven findings, when mapped against our 36-scenario benchmark suite, reveal a systematic pattern: the scenarios with the highest practitioner demand (T4, I5) coincide with the scenarios having zero or minimal tool coverage.
Benchmark categories T4 (advanced training), I5 (production optimizations), and D1 (diffusion) collectively represent 13 of 36 scenarios (36\% of the suite) but account for 0 fully supported scenarios across all five tools.
...
\textbf{Extended tool evaluation.}
Beyond the five primary tools, we attempted deployment of five additional tools to assess the broader ecosystem's accessibility (Table~\ref{tab:extended-deployment}).
MAESTRO~\cite{maestro2019} was the only additional tool that fully built and ran on our platform (macOS ARM64), producing dataflow analysis for ResNet-50 in under one minute---consistent with its CPU-only C++17 design.
Paleo~\cite{paleo2017} installed via pip and produced network architecture summaries, but its simulation and profiling modes failed: cuDNN is required for convolution timing, and the TensorFlow backend requires TF~0.12 (Python~2.7 era), rendering the tool effectively unmaintained since~2018.
ASTRA-sim~\cite{astrasim2023}, Habitat~\cite{habitat2021}, and Accel-Sim~\cite{accelsim2020} all require Linux with NVIDIA GPUs and CUDA toolkits, making them inaccessible on our Apple Silicon test platform.
These platform-unavailability results are themselves informative: they document the hardware barrier that prevents many researchers from reproducing published results, particularly those without access to NVIDIA datacenter GPUs.
In total, we evaluated 10 tools: 5 with full experimental results and 5 with documented deployment outcomes including failure modes.
...
We exclude proprietary tools from evaluation.
The benchmark suite's 36 scenarios, while representative, cannot cover every production deployment pattern; emerging workloads (e.g., retrieval-augmented generation, multi-modal models) are not yet included.

\textbf{Internal validity.}
Our full experiments cover 5 of 25 tools, with deployment testing on 5 additional tools (10 total).
...
Our benchmark suite covers 36 scenarios, but the distribution is not uniform: training scenarios (15) and inference scenarios (18) are complemented by diffusion scenarios (3), with multi-model scenarios (I4) and diffusion scenarios (D1) having the fewest entries per category.
...
We encourage future evaluations to adopt this third-party evaluation methodology and benchmark suite to enable longitudinal tracking of tool accuracy.
...
The MLPerf-Survey-2026 benchmark suite was designed around the LLM and generative-AI workload landscape as of early 2026.
...
Future versions should expand beyond 40 scenarios to maintain coverage as the generative-AI deployment landscape diversifies.
